...
const MovieList: React.FC<MovieListProps> = ({ movies, title, onReload }) => {
    ...
    useEffect(() => {
        if (movies) {
            // Tạo một bản sao và sắp xếp
            const sortedMovies = [...movies].sort((a, b) => {
                // Giả sử trường ngày là 'ngayKhoiChieu'. 
                // Nếu bạn dùng tên khác (ví dụ 'createdAt'), hãy đổi tên biến tương ứng.
                const dateA = new Date(a.ngayKhoiChieu).getTime();
                const dateB = new Date(b.ngayKhoiChieu).getTime();

                // b - a: Giảm dần (Mới nhất lên đầu)
                return dateB - dateA;
            });
