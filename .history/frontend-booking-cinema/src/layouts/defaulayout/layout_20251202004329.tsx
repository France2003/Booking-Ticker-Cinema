...
import { FiSearch, FiMessageCircle, FiArrowUp } from "react-icons/fi"
...
const DefaultLayout = () => {
  ...
  const scrollSmoothToTop = () => {
    const scrollDuration = 600; // thời gian chạy (ms) — muốn chậm tăng lên
      ...
      <div className="default-layout relative ">
        ...
        >
          ...
          <motion.button
            ...
            className="w-14 h-14 flex items-center justify-center rounded-full bg-gradient-to-br from-orange-500 to-red-400 shadow-xl hover:shadow-2xl hover:shadow-orange-400/30">
            <FiMessageCircle className="text-white text-2xl" />
          </motion.button>
          <AnimatePresence>
            {showScrollTop && (
              <motion.button
                onClick={scrollSmoothToTop}
                whileHover={{ scale: 1.2 }}
                whileTap={{ scale: 0.9 }}
                className="w-14 h-14 flex items-center justify-center rounded-full 
    bg-gradient-to-br from-green-500 to-green-700
    shadow-xl hover:shadow-2xl hover:shadow-green-400/30 text-white text-3xl"
              >
                <FiArrowUp />
              </motion.button>
