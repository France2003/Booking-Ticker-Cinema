...
export default function NowShowingPage() {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            try {
                ...
                const filtered = (res?.dangChieu?.data || []).filter(
                    (m: any) => m?.trangThai === "dangChieu" || m?.dangChieu
                )
                setMovies(filtered)
            } catch (err) {
    ...
    return (
        <section className="min-h-screen relative overflow-hidden bg-gradient-to-b from-[#020617] via-[#031c2c] to-[#000] text-white py-16 px-4 sm:px-10 lg:px-20">
            <Helmet>
                <meta charSet="utf-8" />
                <title>Phim đang công chiếu</title>
            </Helmet>
            ...
            <div className="max-w-[1200px] mx-auto relative z-10">
                <motion.h2
                    ...
                    className="text-center text-2xl md:text-4xl font-extrabold uppercase mb-14 text-orange-600  tracking-wider"
                >
                ...
                <div className="text-center mt-12">
                    {visibleCount < movies.length ? (
                        <button
                            ...
                            className="px-8 py-3 rounded-full font-bold text-black bg-[#00E5FF] hover:bg-[#3cf0ff] transition-all duration-200"
                        >
                            Xem thêm
                        </button>
                    ) : movies.length > 6 ? (
                        <button
                            ...
                            className="px-8 py-3 rounded-full font-bold text-black bg-[#00E5FF]/80 hover:bg-[#00E5FF] transition-all duration-200"
                        >
                            Thu gọn
                        </button>
                    ) : null}
